...
/**
 * 모니터링(설계 §6-5): outbox backlog 깊이 + 발송/실패/드롭 카운터.
...
class NotificationMetrics(
    ...
    fun sent() = registry.counter(SENT).increment()
    fun failed() = registry.counter(FAILED).increment()
    fun throttled() = registry.counter(THROTTLED).increment()
